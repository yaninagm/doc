...
\begin{center}
	\begin{longtable}{|p{4.5cm}|p{4cm}|p{4cm}|p{2cm}|}
		\hline \hline \rowcolor[gray]{0.9}
		\multicolumn{4}{||c|}{\textbf{Procedimiento de pruebas}} \\
		\hline 
		
		\multicolumn{4}{||c|}{\textbf{Descripción de escenario:} Debe hacer las pruebas un usuario no logueado} \\
		\hline 
		...
		\hline
		El usuario ingresa la url \textbf{``https:// yesdoc.herokuapp.com/ \#/profileMeasurements''} & El sistema consulta a la API si el token se encuentra activo, la API devuelve un error 401, \textbf{[Figura \ref{no_autorizado_profile_measurement}]}  & Se redirige al usuario al formulario de login & ok\\ 
		\hline
		El usuario ingresa la url \textbf{``https:// yesdoc.herokuapp.com/ \#/profileInformation''} & El sistema consulta a la API si el token se encuentra activo, la API devuelve un error 401  & Se mantiene al redirige al formulario de login & ok\\ 
		\hline
		El usuario ingresa la url \textbf{``https:// yesdoc.herokuapp.com/ \#/home''} & El sistema consulta a la API si el token se encuentra activo, la API devuelve un error 401  & Se redirige al usuario al formulario de login & ok\\ 
		\hline
